...
export function buttonTheme(props: ButtonInterface): Record<string, any> {
  const {
    theme: { color, border, font, unit, utility, Button },
  } = props
...
export function buttonSizeTheme(props: ButtonInterface): Record<string, any> {
  const {
    size,
...
export function buttonStretchTheme(
  props: ButtonInterface
): Record<string, any> {
  const {
    stretch,
