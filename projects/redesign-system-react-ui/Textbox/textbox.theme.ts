...
export function textboxTheme(props: TextboxInterface): Record<string, any> {
  const {
    theme: { utility, border, color, font, unit, Textbox },
  } = props
  ...
  return {
    ...
    transition: `${utility.easeMedium()}, outline`,
    outlineOffset: '-2px',
    width: '100%',
...
export function textboxSizeTheme(props: TextboxInterface): Record<string, any> {
  const {
    size,
    theme: { font, Textbox },
  } = props
...
export function textboxWidthsTheme(
  props: TextboxInterface
): Record<string, any> {
  const {
    widths,
